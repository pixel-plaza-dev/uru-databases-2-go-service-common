...
func (d *DefaultTokenValidator) IsTokenValid(tokenString string, isRefreshToken bool) bool {
	...
	if tokenString != "" {
		ctxMetadata = commongrpcclientctx.NewAuthenticatedCtxMetadata(d.accessToken, tokenString)
	} else {
		ctxMetadata = commongrpcclientctx.NewUnauthenticatedCtxMetadata(tokenString)
	}

	// Get outgoing context
	grpcCtx := commongrpcclientctx.GetCtxWithMetadata(ctxMetadata, context.Background())

	// Validate the token
	if isRefreshToken {
		// Check if the refresh token is valid
		response, err := (*d.authClient).IsRefreshTokenValid(grpcCtx, &pbauth.IsRefreshTokenValidRequest{
	...
	response, err := (*d.authClient).IsAccessTokenValid(grpcCtx, &pbauth.IsAccessTokenValidRequest{
